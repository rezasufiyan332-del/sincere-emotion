...
export default function AdminProductNewPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        toast('Product created', 'The product has been created successfully.', 'success')
        router.push('/admin/products')
      } else {
        toast('Error', data.error?.message || 'Failed to create product', 'destructive')
      }
    } catch {
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-[#64748b]">Add a new guide or product to the store</p>
        </div>
      </div>
